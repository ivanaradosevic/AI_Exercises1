...
def detectEdges(image):
    ...
    canny_image = cv2.Canny(image,110,115)
...
def drawLane(linesLeft, linesRight, frameToDraw):
    ...
    if linesLeft:
        if linesLeft[0][2] == 1:
            ...
            cv2.putText(frameToDraw, "Upozorenje!", (450,100), cv2.FONT_HERSHEY_COMPLEX,2,(0,0,255),4)
            print("Upozorenje")
...
while True:
    ...
    cropped=frame[460:620,:]
    cv2.imshow("krop", cropped)



    # TODO: pozovite funkciju za filtriranje po boji
    RoiFilterd,yellow_mask,white_mask,mask=filterByColor(cropped)
    ...
    # cv2.imshow("roi_color", white_mask)
    # cv2.imshow("zuta", yellow_mask)
    # cv2.imshow("maska", RoiFilterd)
    # cv2.imshow("rubovi", canny_image)
    cv2.imshow("lines", ROILines)
    
    #cv2.imwrite("frame.jpg", ROILines)
